\section{Diseño del Sistema Operativo SerenityOS}
...
\subsection{Componentes implementados en SerenityOS}
...
\paragraph{Parámetros de arranque}
...
\paragraph{Selección de dispositivo de arranque}
...
\paragraph{Componentes clave del Kernel}
...
\paragraph2{Componentes clave en la gestión de procesos}
...
La interfaz de usuario en SerenityOS es única y tiene un enfoque nostálgico hacia los sistemas operativos clásicos de los años 90. Aunque SerenityOS no cuenta con una interfaz de línea de comandos (CLI) tradicional como Bash o Zsh, ofrece una experiencia de usuario basada en ventanas y gráficos que recuerda a los sistemas operativos antiguos. La interfaz de línea de comandos (CLI) es una términal con un shell llamado \texttt{Shell}, que permite a los usuarios interactuar con el sistema operativo mediante comandos de texto, funciona de una manera similar a otros shells de Unix, pero con un enfoque más simple y directo.



\subsubsection{Políticas de planificación y manejo de recursos}
...
El flujo de ejecución del sistema operativo \texttt{SerenityOS} sigue una secuencia estructurada desde el arranque hasta la operación continua del sistema. A continuación se describen las etapas principales del flujo de ejecución:

Previamente al arranque, se necesitan prerequisitos para arrancar la máquina virtual, como tener instalado \textbf{QEMU} y tener las dependencias necesarias para compilar y ejecutar \texttt{SerenityOS}. Para compilar SerenityOS, se utiliza \textbf{CMake 3.25.0.} y \textbf{GCC 11.2.0} o superior. 

La siguiente parte necesita ser instalada: \verb|bash cmake curl e2fsprogs gawk genext2fs git ninja patch python3 qemu rsync| 
\begin{enumerate}
    \item \textbf{Arranque:} Para la ejecución del sistema se necesita el siguiente comando. \verb|Meta/serenity.sh run|
    \item \textbf{Inicialización del kernel:} el kernel configura su subsistema de scheduloing, inicializa controladores básicos de hardware y configura los sistemas de gestión de memoria, manejo de interrupciones y dispositivos, luego se monta el sistema el archivos de raíz ext2 y se configuran sistemas de archivos virtuales como /proc y /dev. Finalmente se inicia 
    \item \textbf{Inicio de  servicios:} El proceso init se encarga de lanzar servicios básicos necesarios para la operación del sistema, como el servidor gráfico y otros servicios esenciales. A medida que el sistema avanza, init gestiona el arranque de servicios adicionales, aunque el enfoque es más sencillo y modular comparado con sistemas operativos más complejos.
